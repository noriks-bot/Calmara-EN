<?php
$main = "/var/www/calmara/si/static/site/products/thera.html";
$html = file_get_contents($main);
$html = preg_replace("#<script[^>]*>[^<]*Appstle[^<]*</script>#is", "", $html);
$html = preg_replace("#<script[^>]*>[^<]*_ALConfig[^<]*</script>#is", "", $html);
$html = preg_replace("#<script[^>]*src=\"[^\"]*shopifycloud[^\"]*\"[^>]*></script>#i", "", $html);
$html = preg_replace("#<script[^>]*src=\"[^\"]*shop\.app[^\"]*\"[^>]*></script>#i", "", $html);
$html = preg_replace("#<script[^>]*src=\"[^\"]*judge\.me[^\"]*\"[^>]*></script>#i", "", $html);
$html = preg_replace("#<script[^>]*src=\"[^\"]*klaviyo[^\"]*\"[^>]*></script>#i", "", $html);
echo $html;
?>
<script>
(function(){
  ...
  document.addEventListener("click", function(e){
    ...
    var PID=getPID(btn), qty=getQty(btn);
    var body=new FormData(); body.append("product_id", PID); body.append("quantity", qty); body.append("add-to-cart", PID);
    fetch("/?wc-ajax=add_to_cart",{method:"POST",body:body,credentials:"include"})
